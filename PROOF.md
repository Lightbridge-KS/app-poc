...
| [`config-driven-shiny`](stacks/dynamic-dashboard/config-driven-shiny) | The PlotSpec is a **human** authoring surface too: the four specs the LLM produced, written as YAML, build to outputs equal float-for-float to the nextjs stack's; the Pydantic and Zod contracts name identical columns and operators; a bad card fails *in its slot* with the field and the legal values while the others render; editing the file updates the page in ~3 s. | The plots' right edge (a few clipped pixels from Plotly sizing before card padding settles), `columns: 1`, many-card files, and malformed-YAML banners. Close it: `just dev`, set `columns: 1`, paste twenty cards, break the indentation, look. |
